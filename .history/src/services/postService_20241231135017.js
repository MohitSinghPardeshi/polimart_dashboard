...
export const updatePostOrderNew = async (postIds,categoryId) => {
    const queryString = postIds.map((id) => `postIds=${id}`).join('&');
    const response = await apiService.put(`/post/v1/post/order?${queryString}&language=HINDI&categoryId=${categoryId}&sangh=Polimart`);
    return response.data; // Assuming a success response
};
